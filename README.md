...
Identifiants : variables `ADMIN_USERNAME` et `ADMIN_PASSWORD`. Compte créé par `seed_data` **une fois** (`docker exec riflet_backend python manage.py seed_data`).
...
6. **Données initiales (première fois uniquement)** — une fois le backend démarré :

```bash
docker exec riflet_backend python manage.py seed_data
```

En production, `seed_data` **n'est plus lancé automatiquement** à chaque redémarrage (`RUN_SEED_DATA=false`). Les modifications faites dans l'admin restent en base entre les mises à jour (volumes Docker `mysql_data` et `media_data`).

### Mises à jour en production (conserver les données)

```bash
docker compose up -d --build
# ou : ./deploy/hostinger.sh update
```

**Ne pas utiliser** `docker compose down -v` — cela **efface** la base MySQL et les fichiers uploadés.

Au redémarrage : **migrations** + **collectstatic** uniquement. Pas de `seed_data`.

### Relancer seed_data manuellement (si besoin)
...
`seed_data` ne crée que ce qui **n'existe pas encore** (compte admin, paramètres par défaut, etc.) — il ne remplace pas le contenu modifié via l'admin.
...
3. Redéployez, puis lancez une fois : `docker exec riflet_backend python manage.py seed_data`
...
À utiliser si MySQL, les médias ou la config sont cassés. **Toutes les données** (base MySQL, photos uploadées, messages) seront **effacées**. Puis relancez `seed_data` manuellement si besoin.
...
Attendez `Seed ignoré (RUN_SEED_DATA=false)` puis lancez une fois : `docker exec riflet_backend python manage.py seed_data`
